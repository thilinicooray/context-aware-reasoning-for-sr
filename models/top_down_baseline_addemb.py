'''
This is the modified baseline model where query encoding is changed to addition instead of
concatenation to be used for verb prediction.
'''
...
class Top_Down_Baseline(nn.Module):
    ...
    def forward(self, v_org, gt_verb):
        ...
        #query is composed by adding verb and role
    ...
    def forward_agentplace_noverb(self, v_org):
        ...
        :return: agent and place role predictions as well as their hidden reps to be used in verb pred.
        '''

        max_role_count = 2 #agent and place
        ...
        role_verb_embd = concat_query.contiguous().view(-1, role_embd.size(-1))
        #during inference time, only role id is used to compose the query as we dont have verb
        q_emb = self.query_composer(role_verb_embd)
